...
public interface IBaseReadRepository<T> where T : BaseModel
{
    ...
    public Task<ICollection<TOut>> GetValuesAsync<TOut>(BaseFilter filter, CancellationToken token = default) where TOut : BaseId;
    public Task<ICollection<T>> GetValuesAsync(BaseFilter filter, CancellationToken token = default);
}